...
- **Sign-in is now magic-link** for her too (used to be instant entry on allowlist match). Type her email at \`/signin\` → if it's on the access list, an email with a one-time sign-in link arrives via Resend → click → 30-day session. Same allowlist gate as before (only listed emails get a link generated), plus the new "prove you control the inbox" step. Rate-limited (3 requests/min per email, 8/min per IP) and anti-enumeration ("Check your email" response regardless of whether the address is on the list). Cookie / session length unchanged.
...
- **What the client sees**: a small "Your space" page with their next upcoming session (with a Join Meet button when it's within 30 min), past sessions list, any outstanding unpaid balance, and how to reach her. Nothing else.
...
## Sign-in
- Type your email on /signin. If it's on the allowlist, we email you a single-use sign-in link via Resend. Click it within 30 minutes; you're in for 30 days. Same allowlist gate as before; the magic link adds an "I control this inbox" step instead of the prior instant-entry behavior.
- Rate limited: 3 link requests per minute per email, 8 per minute per IP. Same "Check your email" response whether the address is allowlisted or not, so an attacker can't enumerate who's on the list.
