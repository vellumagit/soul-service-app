...
- **Public landing page at \`/\`.** The svit.live root URL is now a real landing page for prospective clients instead of bouncing straight to sign-in. Hero + about + how-I-work + what-to-expect + a "Reach out" lead-capture form + quiet email/phone + a small footer ("Already a client? Open your space →" + "Practitioner sign in"). Copy lives on practitioner_settings — she edits all four sections from /settings under a new "Landing page" panel; preview live at \`/?preview=1\`. Signed-in practitioner → auto-redirected to /today. Signed-in portal client → auto-redirected to /portal. Lead submissions land in /network/inbox like any other channel (auto-creates a "Landing page" form on first submission). Honeypot + per-IP rate limit + email normalization. The Today page is now at \`/today\` — sidebar nav + \`g t\` shortcut all point there.
- **Subdomain split: storefront vs workspace.** With \`MARKETING_HOSTNAME=svit.live\` + \`APP_HOSTNAME=app.svit.live\` set in env, the proxy enforces a clean separation: **svit.live** is the public storefront (landing page + inquiry form only — any other path 308-redirects to the app hostname), and **app.svit.live** is where Svit and her clients actually work (workspace + client portal + sign-ins). Svit bookmarks \`app.svit.live/today\`; clients bookmark \`app.svit.live/portal\`. Visitors landing on svit.live see the storefront. When the env vars are unset (dev, preview deploys), the proxy is a no-op and everything works on whatever single host the request hits. Setup steps live in docs/setup-instructions.md → "Step S — Subdomain split."
...
- Seven sections, urgency-ordered. Each has a count chip + a one-line description in her voice:
...
## Sign-in
- Type your email on /signin. If it's on the allowlist (ALLOWED_EMAILS env var), you're in. 30-day session cookie.
...
- Magic-link sign-in is built and ready, but currently OFF — set \`AUTH_REQUIRE_MAGIC_LINK=true\` to enable it (which requires a verified Resend domain so the magic-link emails actually deliver). When on: type email → get one-time link via email → click within 30 min → 30-day session.
...
| Want to | Go to |
|---|---|
| See today + upcoming + tasks | app.svit.live/today (or /today on a single-hostname deploy) |
| Public landing page (her site for prospective clients) | svit.live/ (storefront only; everything else redirects to app.svit.live) |
