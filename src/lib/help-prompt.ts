...
- **Weekly rhythm (recurring auto-schedule)**: on the group detail page there's a "Weekly rhythm" panel — turn on "Automatically schedule this Circle every week," pick a weekday + time, and the app keeps the next ~4 weeks of sessions created on your storefront (deduped, so it never double-books a day you already have a session). It generates the first weeks the instant you save and tops the window up hourly as time passes. Only for published groups; times are in your practice timezone (Settings → Your business) and DST-safe. This is what makes "run a Circle every week and fill seats" hands-free — combined with attendees auto-becoming Network leads, the Circle is a self-running lead engine.
...
- **What's not in this Phase 1 yet (be honest if she asks)**: no per-group email blasts; no waitlist; no recurring auto-generated sessions. All on the backlog. (Card payment via Stripe IS built now — she connects her own account on the Status page; the manual Venmo/cash lane also remains.)
