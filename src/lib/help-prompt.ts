...
- **Circle payments now go straight to your own Stripe account — one-click connect.** On the **Status** page there's a new **"Connect with Stripe"** button. Click it once; Stripe walks you through signing in, adding your bank, and verifying your identity — then anyone can pay for a Circle seat by card. The money lands **directly in YOUR Stripe account** (it doesn't pass through anyone else), the seat confirms itself, and the welcome email with the meeting link sends automatically. Manual Venmo/cash still works alongside it. If Status stays amber right after connecting, Stripe just needs a bit more info before it releases payouts — the page turns green on its own once you finish. To stop taking cards, hit **Disconnect** on the same row.
