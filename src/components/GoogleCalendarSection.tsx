    ...
    <section className="border border-ink-200 rounded-md bg-white p-5">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          ...
          <p className="text-xs text-ink-500 mt-0.5">
            When connected, scheduling a session auto-creates a Calendar event,
            generates a Google Meet link, and emails an invite to your client.
          </p>
        </div>
      </div>
      ...
      {props.flashStatus === "connected" && (
        <div className="mb-3 text-xs text-green-700 bg-green-50 border border-green-100 rounded p-2">
          Connected as <strong>{props.flashEmail || "your Google account"}</strong>.
        </div>
      )}
      ...
      {props.connected ? (
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <ConfirmButton
            ...
            message="Disconnect Google Calendar? Future sessions will need a Meet link pasted manually. Existing events on Google won't be deleted."
      ...
      <div className="mt-4 pt-4 border-t border-ink-100 text-[11px] text-ink-500 leading-relaxed">
        ...
        scheduled here). We don&apos;t read your other calendar events.
      </div>
    </section>
  );
}
